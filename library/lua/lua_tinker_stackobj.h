...
namespace lua_tinker
{
	namespace detail
	{
		...
		struct stack_delay_pop
		{
			lua_State* L;
			int mPop;
			stack_delay_pop(lua_State* nL, int nPop)
				:L(nL), mPop(nPop)
			{}
			~stack_delay_pop() { lua_pop(L, mPop); }
		};
		struct stack_obj
		{
			lua_State* L;
			int _stack_pos;
			stack_obj(lua_State* nL, int nIdx)
				:L(nL), _stack_pos(lua_absindex(L, nIdx))
			{
			}
			~stack_obj() {}
