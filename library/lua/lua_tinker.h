...
namespace lua_tinker
{
	...
	void 	createthread(lua_State *L, const char * thread);
	void 	closethread(lua_State *L, const char * thread);
	void    tdofile(lua_State *L, const char *filename, const char * thread);
	...
	template<typename RVal, typename ...Args>
	RVal tcall(lua_State* L, const char* name, Args&&... arg);
	...
	struct pop0
	{
		T operator () (lua_State * nL)
		{
			L = nL;
			return detail::pop<T>::apply(nL);
		}
	...
	// call lua func
	template<typename RVal, typename ...Args>
	RVal tcall(lua_State* L, const char * thread, const char* name, Args&&... arg)
	{
		lua_pushcclosure(L, get_error_callback(), 0);
		int errfunc = lua_gettop(L);
		
		lua_getfield(L, LUA_REGISTRYINDEX, thread);
		lua_getfield(L, -1, name);
		if (lua_isfunction(L, -1))
		{
			detail::push_args(L, std::forward<Args>(arg)...);
			if (lua_pcall(L, sizeof...(Args), detail::pop<RVal>::nresult, 0) != LUA_OK)
			{
				//remove error info
				lua_pop(L, 1);
			}
